...
`tools/art/generate_font.gd` 现场栅格化：只扫 `assets/i18n/` 下全部 `*.csv` 的翻译表收集字符 → 用 `TextServer` 在关闭抗锯齿、关闭次像素定位下
...
> 新增文案必须先登记进 `assets/i18n/*.csv`，再重跑 `build_assets.sh`；`test_assets.gd` 会检查翻译表字符是否都在字体子集里。
