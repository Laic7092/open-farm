...
### 15.1 一张地图一个场景，但世界是一条链

[code]SceneRouter[/code] 一次只挂一个世界场景（§3.2），所以"一个连续的世界"在实现上
只能是**若干张地图 + 边界的互相引用**。真正的设计问题不是"怎么连"，而是"连成什么形状"：

| 形状 | 代价 |
| --- | --- |
| 星形（每张图都从主城进） | 去哪都要先回主城，走路变成反复横跳 |
| 网状（任意两图直连） | 每加一张图就要跟所有老图对接口，场景文件里全是传送门 |
| **链 + 一条岔路** | 每张图只管左右两个邻居，加图 = 在链上插一段 |

现状就是链：`farm ↔ twon ↔ town ↔ beach`（乡道到海边为止），
矿洞由海滩东北角的洞口进入，村庄另有一条岔路进 `library`；
乡道的两端是农场与海滩，村庄与集市是中间站。`tests/unit/test_world_map.gd` 里
"从农场出发能不能走到每一张图"与"两端各只有一个路口"，就是这条链的可执行版本。

### 15.2 无缝出口：[code]auto_enter[/code] 与 [code]road_exit[/code]

链只是拓扑，玩家感受到的"连成一片"靠的是**边界不打断移动**：

- 地图左右边缘的出口标 [code]auto_enter = true[/code]：走进范围就换图，
  不必对着一格按 E。建筑门口保持默认（按 E 进入），
  否则从商店门口路过就会被吸进去。
- 这些出口同时标 [code]road_exit = true[/code]：它承诺"门压在地图纵向中线上，
  对面那个出生点也在地图纵向中线上"。于是走出农场东口时脚下的路，
  与走进村庄西口时脚下的路在**同一个高度**，人不会"走着走着被甩到地图另一头"。
  这条承诺没有任何运行时机制保证，所以它由单元测试守着。

洞口的处理刻意不同：矿洞入口是海滩东北角的一个"点"（`cave.png` 前面），
不在地图边缘，所以它只有 [code]auto_enter[/code]、没有 [code]road_exit[/code]。

### 15.3 铺地为什么要抽 [code]GroundPainter[/code]

"路要接得上"只有在**每条路都按同一套规则画**时才成立。如果每张地图各写一份铺地代码，
农场东口的乡道与村庄西口的乡道迟早会长成两种宽度、两种高度。
于是路、广场、水面、点缀统一收进 [code]src/world/ground_painter.gd[/code]，
地图脚本只声明自己的构图（`town_ground.gd` 是村庄、`market_ground.gd` 是集市……）。

这里有个与植被系统耦合的坑值得记下来：[code]FloraGrowth.NATURAL_GROUND[/code] 决定
"哪块地能长东西"，而 [code]DIRT[/code] 属于自然地表。所以**土路只能用 [code]PATH[/code]**——
一开始用 [code]DIRT[/code] 拼路肩，结果几天后路面上长出了树。

### 15.4 每张图的分工

| 地图 | 角色 | 特征 |
| --- | --- | --- |
| `farm` | 自家农庄 | 乡道横穿，北侧农舍/畜舍，南侧农田，东南荒地；链的起点 |
| `twon` | 村庄（枢纽） | 主街北侧一排住宅与店铺，南侧广场、水塘、花园、牧场；全部 NPC 住这 |
| `town` | 集市（路过站） | 石板广场 + 摊位 + 水塘；没有常驻 NPC，节日才热闹 |
| `beach` | 海滨 | 沙滩、栈桥、渔船、渔夫小屋；东北角是矿洞洞口 |
| `mine` | 矿洞 | 岩壁封边、主矿道 + 竖井、矿工营地；不下雨下雪 |
| `library` | 图书馆（室内） | 由村庄的门口按 E 进入，不参与乡道网络 |
