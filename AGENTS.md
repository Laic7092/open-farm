...
- **现状**：核心循环 / 玩家 / 农场 / 畜牧 / NPC / UI / 存档 / 本地化全部打通；**美术、字体、BGM、音效全部由脚本生成**，等待填充内容。
...
| 加 NPC / 日程 | `data/npcs/` `data/dialogue/` `data/schedules/` + `src/npc/` + 场景里的 `SchedulePoint` |
| 加地图 | 复制 `scenes/world/twon.tscn`，根用 `WorldScene`，放 `SpawnPoint`，用 `SceneDoor` 互连 |
...
- **九个单例（顺序 = `project.godot` 声明顺序）**：
  `EventBus` `AppTheme` `Database` `GameClock` `GameState` `WeatherSystem` `SaveManager` `SceneRouter` `Audio`。
