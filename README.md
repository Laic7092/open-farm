...
游戏是**纯键盘操作**：鼠标指针已隐藏，指针事件被吞掉。菜单 / 商店 / 标题页统一用
`W A S D` 或方向键导航，`回车` / `空格` /`E` 确认，`Esc` 返回。商店里
`A` / `D` 在"购买 / 卖出"两个列表之间切换。
...
出生在自家农舍门口。往下是出货箱，往南是农田区（带栅栏的那片），沿乡道往东出农场就是村庄。
用锄头翻地 → 浇水 → 用 `seed_bag`（物品栏第 4 格）播种 → 等作物成熟后按 `E` 徒手收获。
...
地图网络是一条**能一路走下去的链**：农场 `farm` → 村庄 `twon` → 集市 `town` →
海滩 `beach` → 矿洞 `mine`，村庄另有一条岔路通图书馆 `library`。
每张图左右两侧的出口都压在地图的**纵向中线**上，且 `SceneDoor.auto_enter` 为真——
走到地图边缘就换图（不必对着格子按 E），落脚点正在对面那条路的同一高度上，
于是"从自家农场一路往东走到海边"是一段连贯的路，而不是几次互不相干的传送。
...
2. 地面用脚本铺：户外走 `GroundPainter`（田野参考 `src/world/town_ground.gd`、
   集市参考 `market_ground.gd`、海滩 / 矿洞各有自己的 `*_ground.gd`），
...
4. 在两边用 `SceneDoor` 互相连接（`target_scene` + `target_spawn_id`）。
   地图边缘的出口标 `auto_enter = true`（走进即传送），建筑门口留默认值（按 E 进入）；
   横向出口与对面的出生点都放在地图纵向中线上——
   `tests/unit/test_world_map.gd` 会检查这条规则，也检查"从农场能走到每一张图"。
...
| 层次 | 工具 | 覆盖 |
| --- | --- | --- |
| 单元测试 | gdUnit4（`tests/unit/`，343 例） | 日期进位、季节/天气、网格换算、背包堆叠、体力、物品栏、作物生长（含枯死/多次收获）、牲畜养殖（成年/产出/喂食/好感度）、NPC 日程表与网格 A*、商店经济、状态机、时钟与日结转钩子、好感度与恋爱（聊天/送礼/表白/结婚/生子）、节日规则（日期/开放时间/倒数）与事件条件、节日与事件的运行时状态（参加奖励每年一次、事件一次性触发、存档往返）、数据完整性、存档往返与容错、**世界连通性（从农场可达每张图、每个出口的落地出生点存在、乡道出口压在地图纵向中线上）** |
| 冒烟测试 | `tools/smoke_test.tscn`（286 项） | 真的把游戏跑起来：场景加载、玩家落点、翻地→播种→生长→收获全链路、放养→喂食→成长→收产出、买/卖、存读档、HUD 内容、**NPC 日程与寻路（导航网格、路径、真的走起来）**、**好感度 / 恋爱 / 婚姻链路**、**昼夜光照（环境光随时刻变化、路灯白天灭夜里亮）**、**沿"农场 → 村庄 → 集市 → 海滩 → 矿洞 → 图书馆 → 农场"巡游一圈后农田与畜舍进度、日结转钩子仍然有效**、**六张地图加载、出口接线（目标场景 / 目标出生点）与 NPC 导航可达性**、**真的走进农场东口就换到村庄（无缝出口）**、**节日与事件（今日节日播报、HUD 横幅、会场按时间开放、参加奖励与存档往返、非节日当天会场收摊）** |
...
| 视觉回归 | `tools/screenshot.tscn` / `tools/ui_preview.tscn` | 标题页 + 农场 + 村庄 + 集市 + 海滩 + 矿洞 + 图书馆截图、各界面布局截图 |
